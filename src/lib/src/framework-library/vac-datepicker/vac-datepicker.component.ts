...
@Component({
  ...
  template: `      
    <div style="padding-bottom: 1.25em;">
        ...
        <mat-hint ngClass="mat-hint" *ngIf="options?.description && (!options?.showErrors || !options?.errorMessage)"
...
export class VacDatepickerComponent implements OnInit, OnChanges {
  ...
  updateValue(event) {
        ...
        if(!event.valid && event.value != ""){
            this.options.errorMessage = this.options.validationMessages.format({"requiredFormat":this.options.dateFormat});
